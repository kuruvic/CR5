...
void loop() 
{
     ...
     if(_MODE == "FRONT")
     {
         ...
         if((encoder1count >= uStep.countL)&&(encoder2count >= uStep.countR)) 
         _MODE = "STOP";
     }
     ...
     if(_MODE  == "START")
     {
        ...
        else if(clean == true)
        {
            ...
            if(_MODE3 == "RT")
            {
                if((adcdata[3]>=250)||(adcdata[6]>=250)|(digidata_1>=1)||(digidata_2>=1))
                {
                    _MODE3 = "RGR";
                    _MODE2 = "BACK";
                    clearEncoderCount();
...
{
    if((2 < encoder2count - encoder1count)||(Zangle-angz <= -5))
    {
        spdl = slp + 0.2;
        spdr = srp - 0.2;
    }

    else if((2 < encoder1count - encoder2count)||(Zangle-angz >= 5))
    {
        spdl = slp - 0.2;
        spdr = srp + 0.2;
    }
...
{
    if(-2 > encoder2count - encoder1count)//||(Zangle >= 5))
    {
         spbl = slb + 0.2;
         spbr = srb - 0.2;
    }

    else if(-2 > encoder1count - encoder2count)//||(Zangle <= -5))
    {
         spbl = slb - 0.2;
         spbr = srb + 0.2;
    }
